...
import sellerModel from "../models/sallerModel.js";
...
// seller - registercontroler
export const SellerRegisterController = async (req, res) => {
  try {
    const { name, email, password, phone, address, answer, shopType, photo } = req.body;
    //validations
    if (!name) {
      return res.send({ error: "Name is Required" });
    }
    ...
    if (!shopType) {
      return res.send({ message: "Shope Type is Required" });
    } if (!photo) {
    ...
    if (exisitingSeller) {
      return res.status(200).send({
        success: false,
        message: "Already You are Seller please login",
      });
    }
    //register seller
    const hashedPassword = await hashPassword(password);
    ...
    res.status(201).send({
      success: true,
      message: "your Register Successfully as a Seller Now You can Sale Your Product on Deals Hunt",
      seller,
    });
  } catch (error) {
    console.log(error);
